...
describe("suggest", () => {
  it("madonna returns ray of light", async () => {
    const response = await chakram.get(
      "http://service/suggest/artists?prefix=madon"
    );
  ...
  it("world tracks", async () => {
    const response = await chakram.get(
      "http://service/suggest/tracks?prefix=world"
    );
    const worldTracks = {
      suggestions: [
        {
  ...
  it("dream releases", async () => {
    ...
    const response = await chakram.get(
      "http://service/suggest/releases?prefix=dream"
    );
    expect(response).to.have.status(200);
